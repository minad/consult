...
(defun consult--multi-lookup (sources selected candidates _input narrow &rest _)
  ...
  (if (or (string-blank-p selected)
      ...
      (let* ((src
              (or (and narrow (seq-find (lambda (s)
                                          (let ((n (plist-get s :narrow)))
                                            (eq (or (car-safe n) n -1) narrow)))
                                        sources))
                  (seq-find (lambda (s) (plist-get s :default)) sources)
                  (seq-find (lambda (s) (and (plist-get s :new)
                                               (not (plist-get s :hidden)))) sources)
                  (seq-find (lambda (s) (not (plist-get s :hidden))) sources)
                  (aref sources 0)))
