...
#+html: <a href="https://releases.melpa.org/#/consult"><img alt="MELPA Releases" src="https://releases.melpa.org/packages/consult-badge.svg"/></a>
#+html: <a href="https://snapshots.melpa.org/#/consult"><img alt="MELPA Snapshots" src="https://snapshots.melpa.org/packages/consult-badge.svg"/></a>
...
- =:items= List of strings to select from or function returning list of strings.
  The function is only called for visible sources, such that candidates are only
  computed on demand. The strings can carry metadata in text properties, which
  is then available to the =:annotate=, =:action= and =:state= functions. The list can
  also consist of pairs, with the string in the =car= used for display and the =cdr=
...
via the [[https://github.com/mhayashi1120/Emacs-wgrep][wgrep]] package (or the less featureful =grep-edit-mode= on Emacs 31). These
three workflows are symmetric.

+ =consult-line= -> =embark-export= to =occur-mode= buffer -> =occur-edit-mode= for editing of matches.
+ =consult-grep= -> =embark-export= to =grep-mode= buffer -> =wgrep-change-to-wgrep-mode= for editing of matches.
...
Consult can be installed from [[https://elpa.gnu.org/packages/consult.html][ELPA]] or [[https://releases.melpa.org/#/consult][MELPA]] via the Emacs built-in package
...
By default Consult operates on all buffers in your Emacs instance, as returned
by =(buffer-list)=. The variable =consult-buffer-list-function= specifies a function
which returns the list of buffers. This function is called for instance by
=consult-buffer= sources. The variable variable can be set to alternative
functions.

For per-frame/per-tab isolation set =consult-buffer-list-function= to
=consult--frame-buffer-list=. Alternatively use special buffer functions provided
by workspace packages like [[https://github.com/protesilaos/beframe][beframe]], [[https://codeberg.org/mclear-tools/tabspaces][tabspaces]] or [[https://github.com/florommel/bufferlo][bufferlo]]. This way Consult
integrates with your favored solution for workspaces and buffer isolation.
...
- [[https://github.com/mhayashi1120/Emacs-wgrep][wgrep]] (or the less featureful =grep-edit-mode= on Emacs 31): Editing of grep
...
- [[https://github.com/protesilaos/mct][Mct]] by Protesilaos: Minibuffer and Completions in Tandem, which builds on the
...
first. If you have a proposal, take a look at the [[https://github.com/minad/consult/issues][Consult issue tracker]] and the
[[https://github.com/minad/consult/issues/6][Consult wishlist]]. There have been many prior feature discussions. Please search
through the issue tracker, maybe your issue or feature request has already been
...
Scvorțov]], [[https://github.com/tecosaur][Tecosaur]], [[https://github.com/thisirs][Sylvain Rousseau]], [[https://github.com/tomfitzhenry][Tom Fitzhenry]], [[https://hg.serna.eu][Iñigo Serna]] and [[https://github.com/akreisher][Alex Kreisher]].
