...
** Asynchronous search
 ...
 filtering. There are two multple splitting styles available, configured in
 ~consult-async-split-styles-alist~, e.g., =space=, =comma=, =semicolon= and
 =perl=. The default splitting style is configured with the variable
