...
=consult-preview-variables= and =consult-preview-allowed-hooks=. Only mode hooks
(e.g. =prog-mode-hook=) and find file hooks (=find-file-hook=) listed in
=consult-preview-allowed-hooks= are executed. In order to enable additional font
locking during preview, add the corresponding hooks to the allow list. The
following code demonstrates this for [[https://github.com/minad/org-modern][org-modern]] and [[https://github.com/tarsius/hl-todo][hl-todo]].
