...
** Asynchronous search
 ...
 filtering. There are two splitting styles available: =punctuation= and =space=. It
 is possible to configure the splitting style with the variable
 ~consult-async-split-style~.

 For the =space= splitting style, the first word is passed to grep, the remaining
 string is used for filtering.

 For the =punctuation= splitting style, the input string is split at a
 punctuation character, using a similar syntax as Perl regular expressions.
 ...
 *TIP:* If you have [[https://github.com/minad/marginalia][Marginalia]] installed, type =M-x customize-variable RET
 ...
 | consult-async-split-style     | '(punctuation "#") | Splitting style used for async commands               |
