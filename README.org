...
There exist other completion UIs beside Vertico, which are supported by Consult.
...
- [[https://github.com/bdarcus/citar][citar]]: Versatile package for citation insertion and bibliographies.
...
2. Either use the default completion UI or ensure that exactly one of
   =vertico-mode=, =selectrum-mode=, =mct-mode=, or =icomplete-mode= is enabled. (Use
   =icomplete-mode= instead of =fido-mode=, since Fido always overwrites the
   =completion-styles= style with =flex=. Furthermore =ivy-mode= and =helm-mode= must be
   disabled.)
...
- Your completion UI (Default completion, Vertico, Selectrum, Mct or Icomplete).
