...
** Register
 ...
 - =consult-register-store=: Improved UI to store registers depending on the current
   ...
   * =M-' x=: If a region is active, store region in register =x=.
...
:properties:
:description: Instructions on how to create reproducible bugreports
:end:
...
Embark and Icomplete-vertical, in case you are using these packages.
...
Please try to reproduce the issue by starting Emacs with =emacs -Q= on the
command line and executing the following code in the scratch buffer. This way we
can exclude side effects due to configuration settings.
