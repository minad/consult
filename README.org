...
Note that there are two packages as of now: =consult.el= and =consult-flycheck.el=.
Consult has been split such that the main package =consult.el= only depends on Emacs core
components.
