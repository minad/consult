...
Multi-path and wildcard handling contributed by Robert Weiner (@rswgnu):
- Add =consult--search-path-list= to store multiple paths per search.  This can
  be any number of directories followed by any number of file paths.
- Add =consult--set-dir-and-paths= to support grep over multiple directories
  and wildcarded paths.
- Change =consult--directory-prompt= with prefix arg to prompt for a string
  that can contain multiple wildcard/glob expressions and/or directories to
  recursively search.  Also, save dir value as a list into
  =consult--search-path-list=.
- Add public =consult-last-command-args= and =consult-this-command-args= variables
  that store prior and current list of shell command args run by consult.  Useful
  for rerunning commands or branching based on prior command args.
