...
- Add support for grep and find over multiple directories and wildcarded paths.
- Change =consult--directory-prompt=.  With a prefix arg sent as =dir= to one
  of the consult search commands:
    ...
    with any other prefix arg, prompt for a space-separated string of any
    number of directories followed by file paths optionally with wildcards.
