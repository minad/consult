...
- =consult-grep=: Add support for grep and find over multiple space or comma
  separated files and/or directories. If the prefix argument DIR is a single
  C-u, prompt for a directories or files to search recursively via
