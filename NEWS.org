#+title: consult.el - News
#+author: Daniel Mendler
#+language: en

* Version 3.9 (2026-09-13)

- =consult-buffer=, =consult--multi=: Populate sources lazily, such that expensive
  hidden sources are not computed unnecessarily.
- =consult-theme=: Group themes by package.
- Minor internal refactoring.

* Version 3.8 (2026-09-03)

- =consult-completion-in-region=: Support default completion UI if eager display
  and update is enabled, such that in-buffer completion can be transferred to
  the minibuffer.
- =consult-focus-lines=: Handle revert buffer.

* Version 3.7 (2026-08-05)

- =minibuffer-exit-hook=: Internal refactoring of minibuffer-local hooks.
- Use =completion-list-candidate-at-point= from Compat 31.

* Version 3.6 (2026-06-05)

- =consult--command-splitter=: Require at least a single character behind the
  dash to recognize an option.
- =consult-find=: Support =-name=, =-iname=, =-regex= and =-iregex= options.

* Version 3.5 (2026-05-19)
...
- Remove obsolete =consult-preview-at-point-mode= and =consult-preview-at-point=.
